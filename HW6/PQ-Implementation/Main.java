...
class PriorityQueue<E, P> {
    ...
    public Node add(E e, P priority) {
        // If the tree is empty we insert at the root and return
        if (tree.isEmpty()) {
            Node node = new Node(e, priority, 0);
            tree.add(node);
            return node;
        } else {
    ...
    public boolean contains(E e) {
        // Loop over the priority queue and check if a certain node is 
        // Equal to the element were looking for.
        for (Node n : tree) {
            if (n.value().equals(e)) {
        ...
        // Other we return false, meaning not contained in the PQ.
        return false;
    }
