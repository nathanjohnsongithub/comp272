...
class LUC_AVLTree {
    ...
    private Node deleteElement(int value, Node node) {
        ...
        if (value == node.value) {
            // need to do some type of delete because we've found the node
            if (node.leftChild == null && node.rightChild == null) {
                // essentially deleting
                return null;
            ...
            int BF = getBalanceFactor(node);
            if (Math.abs(BF) > 1) {
                // we know we need to do a rotation
                if (BF > 1) {
                    // left rotation
                    int leftBF = getBalanceFactor(node.leftChild);
                    if (leftBF > 1) {
                        node = LLRotation(node);
                    } else {
                        node = LRRotation(node);
                    ...
                    // right rotation
                    int rightBF = getBalanceFactor(node.rightChild);
                    if (rightBF < 0) {
                        node = RRRotation(node);
                    } else {
                        node = RLRotation(node);
